...
function toneClass(tone, compact, variant) {
  ...
  if (variant === 'ink') {
    if (tone === 'primary') {
      return `${anim.shineParchment} inline-flex items-center gap-1.5 rounded-full bg-parchment ${pad} font-bold text-teal-950`;
    }
    if (tone === 'amber') {
      return `inline-flex items-center gap-1.5 rounded-full border border-parchment/35 bg-white/10 ${pad} font-bold text-parchment`;
    }
    if (tone === 'soft') {
      return `inline-flex items-center gap-1.5 rounded-full border border-parchment/25 ${pad} font-semibold text-parchment/80`;
    }
    return `inline-flex items-center gap-1.5 rounded-full border border-parchment/35 bg-white/10 ${pad} font-bold text-parchment`;
  }
  if (tone === 'primary') {
    return `${anim.shine} inline-flex items-center gap-1.5 rounded-full bg-teal-800 ${pad} font-bold text-white`;
  }
  if (tone === 'amber') {
    return `inline-flex items-center gap-1.5 rounded-full border border-amber-500/35 bg-amber-50 ${pad} font-bold text-amber-950`;
  }
  if (tone === 'soft') {
    return `inline-flex items-center gap-1.5 rounded-full border border-ink/15 bg-white/80 ${pad} font-semibold text-ink/70`;
  }
  return `inline-flex items-center gap-1.5 rounded-full border border-teal-700/25 bg-white ${pad} font-bold text-teal-950`;
}
