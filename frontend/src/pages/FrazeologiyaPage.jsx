...
import { fetchFrazeologiyaById, searchFrazeologiya } from '../api/dicts';
import StructuredSenses from '../components/dictionary/StructuredSenses';

const PAGE_SIZE = 40;

export function FrazeologiyaDetail() {
  const { id } = useParams();
  const { text } = useUiScript();
  const { status, data, error, reload } = usePageData(
    () => loadPageBundle({ entry: () => fetchFrazeologiyaById(id).then((r) => r.data) }),
    { deps: [id] }
  );
  const entry = data?.entry;
  usePageMeta(entry?.phrase || 'Frazeologizmler', entry?.gloss?.slice(0, 120));

  return (
    <PageGate status={status} error={error} onRetry={reload} backHref="/dictionary/frazeologiya">
      {entry && (
        <DictShell className="pt-24 pb-24">
          <div className="relative mx-auto max-w-3xl px-5 md:px-8">
            <Link to="/dictionary/frazeologiya" className="text-sm text-ink/45 hover:text-teal-900">
...
export default function FrazeologiyaPage() {
  const { text } = useUiScript();
  ...
  usePageMeta('Frazeologizmler sózligi', 'Qaraqalpaq tiliniń frazeologizmleri');
      ...
      <DictShell className="pt-24 pb-24">
        <div className="relative mx-auto max-w-4xl px-5 md:px-8">
          <div className="mb-6 flex flex-wrap items-center gap-3 text-sm">
            ...
            <span className="font-medium text-ink/70">{text('Frazeologizmler')}</span>
          </div>
          ...
          >
            <input
              ...
              placeholder={text('Fraza yamasa mánisi…')}
          ...
          <ul className="space-y-2">
            {rows.map((row) => (
              <li key={row.id}>
                <button
                  type="button"
                  onClick={() => navigate(`/dictionary/frazeologiya/${row.id}`)}
