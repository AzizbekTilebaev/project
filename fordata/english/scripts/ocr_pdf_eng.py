#!/usr/bin/env python3
"""OCR a scanned English textbook PDF (Tesseract eng)."""
...
# Reuse project tessdata (eng.traineddata lives with kaa)
TESSDATA = ROOT.parent / "grammar" / "tessdata"
...
def assemble(out: Path, pdf: Path, total: int, dpi: int, lang: str) -> None:
    parts = [
        f"===== OCR {out.name} =====",
        f"Source: {pdf}",
        f"Lang: {lang} | DPI: {dpi} | Pages: {total}",
...
def main() -> int:
    ...
    if not pdf.is_file():
        ...
        return 1
    eng = TESSDATA / f"{args.lang}.traineddata"
    if not eng.is_file():
        print(f"Missing {eng}", file=sys.stderr)
        return 1
    ...
    log(
        f"START {args.slug} pages={total} dpi={args.dpi} jobs={args.jobs} lang={args.lang}",
        log_path,
    )
    ...
    assemble(out, pdf, total, args.dpi, args.lang)
    nonempty = sum(1 for p in pages_dir.glob("*.txt") if p.stat().st_size > 0)
    log(
        f"DONE {args.slug} nonempty={nonempty}/{total} ok_calls={ok} -> {out / 'full.txt'}",
        log_path,
    )
    return 0
